...
BLERemoteCharacteristic::BLERemoteCharacteristic(uint16_t con, const void /*gatt_client_characteristic_t*/ *cha) {
    ...
    DEBUGBT("%s", _uuid.toString().c_str());
...
void BLERemoteCharacteristic::packetHandler(uint8_t type, uint16_t channel, uint8_t *packet, uint16_t size) {
    ...
    case GATT_EVENT_CHARACTERISTIC_VALUE_QUERY_RESULT:
        DEBUGBT("GATT_EVENT_CHARACTERISTIC_VALUE_QUERY_RESULT");
    ...
    case GATT_EVENT_ALL_CHARACTERISTIC_DESCRIPTORS_QUERY_RESULT:
        DEBUGBT("GATT_EVENT_ALL_CHARACTERISTIC_DESCRIPTORS_QUERY_RESULT");
    ...
    case GATT_EVENT_CHARACTERISTIC_DESCRIPTOR_QUERY_RESULT:
        DEBUGBT("GATT_EVENT_CHARACTERISTIC_DESCRIPTOR_QUERY_RESULT");
    ...
    case GATT_EVENT_NOTIFICATION:
        DEBUGBT("GATT_EVENT_NOTIFICATION");
        if (gatt_event_notification_get_value_handle(packet) == _gattCharacteristic->value_handle) {
            ...
            DEBUGBT("Got unexpected notification CB!");
        }
        break;
...
String BLERemoteCharacteristic::description() {
    ...
    if (!d || !con_handle) {
        DEBUGBT("no secrription descriptor found");
        free(_description);
...
bool BLERemoteCharacteristic::enableNotifications() {
    if (!canNotify()) {
        DEBUGBT("can't notify");
        return false;
    }
    if (_notifyOn) {
        DEBUGBT("notify already on");
        return true;
    }
    volatile bool waitForCB = true;
    _cbflag = &waitForCB;
