...
#define pio_tdm_in_swap_wrap_target 0
#define pio_tdm_in_swap_wrap 3

static const uint16_t pio_tdm_in_swap_program_instructions[] = {
            //     .wrap_target
    0xb822, //  0: mov    x, y            side 3     
    0x4001, //  1: in     pins, 1         side 0     
    0x1041, //  2: jmp    x--, 1          side 2     
...
#if !PICO_NO_HARDWARE
static const struct pio_program pio_tdm_in_swap_program = {
...
static inline pio_sm_config pio_tdm_in_swap_program_get_default_config(uint offset) {
    pio_sm_config c = pio_get_default_sm_config();
    sm_config_set_wrap(&c, offset + pio_tdm_in_swap_wrap_target, offset + pio_tdm_in_swap_wrap);
    sm_config_set_sideset(&c, 2, false, false);
    return c;
}
#endif
...
static inline void pio_tdm_in_program_init(PIO pio, uint sm, uint offset, uint data_pin, uint clock_pin_base, uint bits, bool swap, uint channels) {
    pio_gpio_init(pio, data_pin);
    pio_gpio_init(pio, clock_pin_base);
    pio_gpio_init(pio, clock_pin_base + 1);
