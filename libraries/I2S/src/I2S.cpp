...
bool I2S::begin() {
    ...
    if (!_swapClocks) {
        _i2s = new PIOProgram(_isOutput ? 
            ...
            (_isTDM ? &pio_tdm_in_program : &pio_i2s_in_program));
    } else {
        _i2s = new PIOProgram(_isOutput ? 
            (_isInput ? 
            ...
            (_isTDM ? &pio_tdm_in_swap_program : &pio_i2s_in_swap_program));
    }
    int minpin, maxpin;
    ...
    if (_isOutput) {
        ...
        if (_isTDM) {
            pio_tdm_in_program_init(_pio, _sm, off, _pinDIN, _pinBCLK, _bps, _swapClocks, _tdmChannels);
        } else {
