...
void __retarget_lock_acquire(_LOCK_T lock) {
    if (__freeRTOSinitted && vTaskPreemptionDisable) {
...
int __retarget_lock_try_acquire(_LOCK_T lock) {
    ...
    auto ret = mutex_try_enter((mutex_t *)lock, NULL);
    if (!ret && __freeRTOSinitted && vTaskPreemptionEnable) {
...
int __retarget_lock_try_acquire_recursive(_LOCK_T lock) {
    ...
    auto ret = recursive_mutex_try_enter((recursive_mutex_t*)lock, NULL);
    if (!ret && __freeRTOSinitted && vTaskPreemptionEnable) {
        vTaskPreemptionEnable(nullptr);
    }
    return ret;
...
void __retarget_lock_release(_LOCK_T lock) {
    ...
    if (__freeRTOSinitted && vTaskPreemptionEnable) {
        vTaskPreemptionEnable(nullptr);
    }
}
