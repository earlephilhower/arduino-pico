...
def MakeBoard(name, chip, vendor_name, product_name, vid, pid, pwr, boarddefine, flashsizemb, psramsize, boot2, extra = None, board_url = None):
    ...
    BuildUSBStack(name)
    if name in ["rpipicow", "rpipico2w", "pimoroni_pico_plus_2w", "sparkfun_thingplusrp2350", "waveshare_rp2350_plus_w"]:
        BuildCountry(name)
    BuildIPBTStack(name)
    if name == "generic":
        BuildBoot(name)
...
MakeBoard("waveshare_rp2350_plus_w", "rp2350", "Waveshare", "RP2350 Plus W", "0x2e8a", "0x0009", 800, "WAVESHARE_RP2350_PLUS_W", 16, 0, "none", ["PICO_CYW43_SUPPORTED=1", "CYW43_PIN_WL_DYNAMIC=1"])
